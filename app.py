...
class Participant(db.Model):
    ...
    had_bye = db.Column(db.Boolean,default=False)
...
def swiss_pairings_participants(participants, round_number):
    ...
    def select_bye_player(players):
        """Select bye recipient - lowest score, hasn't had bye; tie-break on higher id (like your code)."""
        eligible = [p for p in players if not getattr(p, 'had_bye', False)]
        if not eligible:
            eligible = players[:]
...
def save_round_results(tournament_id, round_number, form_data):
    ...
    # Track all bye players (including pre-existing ones from generation)
    ...
    for match in pairings:
        ...
        if winner == "white":
            ...
            white.had_bye = True
            white.last_colors.append('white')
            white.float_history.append('bye')
            match['result'] = "bye_white"
            # Add to bye list if not already there
            ...
            black.had_bye = True
            black.last_colors.append('black')
            black.float_history.append('bye')
...
def get_standings(tname):
    ...
    for p in participants:
        ...
        had_bye = getattr(p,'had_bye',False)
    ...
    standings.sort(key=lambda x: (-x['score'], -x['buchholz_cut1'], -x['buchholz'],-x['elo'], -x['black_count'],x['had_bye']))
